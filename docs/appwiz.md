...
- **Remove** — only for modules declared by the application. A dependency of
  another module ("required by module chicago.shell") and a module that lies
...
`CHICAGO_DEPS_FS` — the identifier of an `fs.directory` entry over the
...
The window's permissions (`chicago.appwiz:window_scope`): read the
...
(`chicago.shell.sdk:app`): a table with columns, buttons, an input line — shared components,
both render modes.
